...
def mind4(mnr, rea, liste_d4, basisaar, startaar, freq='M'):
    ...
    if len(mnrzerowarnlist) > 0:
        warnings.warn(f'There are only zeroes in {mnrzerowarnlist} in the monthly dataframe.', UserWarning, stacklevel=2)

    mnrintwarnlist = []
    for col in mnr_of_concern.columns:
        if not pd.api.types.is_any_real_numeric_dtype(mnr_of_concern[col]) and col not in mnr_of_concern.columns[mnr_of_concern.isna().any()].to_list():
            mnrintwarnlist.append(f'{col}')
    if len(mnrintwarnlist) > 0:
        warnings.warn(f'There are values in {mnrintwarnlist} in the {periodely} dataframe that are not real numbers. Skipping sending these to benchmarking.', UserWarning, stacklevel=2)
    ...
    if len(reazerowarnlist) > 0:
        warnings.warn(f'There are only zeroes in {reazerowarnlist} in the yearly dataframe.', UserWarning, stacklevel=2)

    reaintwarnlist = []
    for col in rea_of_concern.columns:
        if not pd.api.types.is_any_real_numeric_dtype(rea_of_concern[col]) and col not in rea_of_concern.columns[rea_of_concern.isna().any()].to_list():
            reaintwarnlist.append(f'{col}')
    if len(reaintwarnlist) > 0:
        warnings.warn(f'There are values in {reaintwarnlist} in the yearly dataframe that are not real numbers. Skipping sending these to benchmarking.', UserWarning, stacklevel=2)
    # reaintwarnlist
    rea_of_concern = rea_of_concern[[col for col in rea_of_concern.columns if col not in reaintwarnlist]]
    mnr_of_concern = mnr_of_concern[[col for col in mnr_of_concern.columns if col not in reaintwarnlist]]
    #mnrintwarnlist
    mnr_of_concern = mnr_of_concern[[col for col in mnr_of_concern.columns if col not in mnrintwarnlist]]
    rea_of_concern = rea_of_concern[[col for col in rea_of_concern.columns if col not in mnrintwarnlist]]
    liste_d4 = [serie for serie in liste_d4 if serie not in reaintwarnlist and serie not in mnrintwarnlist]
    ...
    for elem in liste_d4:
        ...
        print(f'Benchmarking {printlist} with MinD4 from {startaar} to {basisaar}.', end="\r", flush=True)
    ...
    skippe = mnr_of_concern.columns[mnr_of_concern.isna().any()].to_list() + rea_of_concern.columns[rea_of_concern.isna().any()].to_list()
