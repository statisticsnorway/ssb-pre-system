...
def convert(input_df: pd.DataFrame, to_freq: str) -> pd.DataFrame:
    ...
    input_periods_per_year = freq_to_periods_per_year.get(input_df.index.freqstr[0].lower())
    ...
    if input_periods_per_year is None:
        raise ValueError(f'cannot convert from frequency {input_df.index.freqstr[0].lower()}')
