...
def test_mind4_monthly_happy_path_yearly_sums_match():
    ...
    res = mind4(mnr=mnr, rea=rea, liste_d4=['A', 'B'], basisaar=2021, startaar=2020, freq='M')
...
def test_mind4_quarterly_happy_path_yearly_sums_match():
    ...
    idx_y = _mk_period_index('2020', '2021', 'Y')
    rea = pd.DataFrame({
        'A': [mnr_q.loc[mnr_q.index.year == 2020, 'A'].sum(),
              mnr_q.loc[mnr_q.index.year == 2021, 'A'].sum()],
    }, index=idx_y)

    res = mind4(mnr=mnr_q, rea=rea, liste_d4=['A'], basisaar=2021, startaar=2020, freq='Q')
    res_y = res.resample('Y').sum()
...
def test_mind4_input_validation_errors():
    ...
    with pytest.raises(TypeError):
        mind4(mnr=mnr.reset_index(), rea=rea, liste_d4=['A'], basisaar=2020, startaar=2020)
    with pytest.raises(TypeError):
    ...
    with pytest.raises(TypeError):
        mind4(mnr=mnr, rea=rea[['A']].rename(columns={'A': 'B'}), liste_d4=['A'], basisaar=2020, startaar=2020)
    with pytest.raises(TypeError):
        mind4(mnr=mnr[['A']].rename(columns={'A': 'B'}), rea=rea, liste_d4=['A'], basisaar=2020, startaar=2020)
    with pytest.raises(TypeError):
...
def test_mind4_emits_warnings_and_skips_non_numeric():
    ...
    with pytest.warns(UserWarning):
        res = mind4(mnr=mnr, rea=rea, liste_d4=['A', 'B', 'C', 'D'], basisaar=2020, startaar=2020, freq='M')
